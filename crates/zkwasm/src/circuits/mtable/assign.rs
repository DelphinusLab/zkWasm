                ...
                let _ = entries
                    ...
                    .map(|(entry, rest_mops)| {
                        ...
                        match entry.entry.ltype {
                            LocationType::Stack => {
                                self.config.is_stack_cell.assign(ctx, F::one())?
                            }
                            LocationType::Heap => self.config.is_heap_cell.assign(ctx, F::one())?,
                            LocationType::Global => {
                                self.config.is_global_cell.assign(ctx, F::one())?
                            }
                        };
                        ...
                        if entry.entry.atype.is_init() {
                            ...
                            self.config
                                .offset_align_left
                                .assign(ctx, F::from(left_offset as u64))?;
                            self.config
                                .offset_align_right
                                .assign(ctx, F::from(right_offset as u64))?;
                            self.config
                                .offset_align_left_diff_cell
                                .assign(ctx, F::from((entry.entry.offset - left_offset) as u64))?;
                            self.config
                                .offset_align_right_diff_cell
                                .assign(ctx, F::from((right_offset - entry.entry.offset) as u64))?;
