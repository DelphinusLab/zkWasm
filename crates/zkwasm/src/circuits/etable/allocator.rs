...
pub(crate) struct AllocatedMemoryTableLookupReadCell<F: FieldExt> {
    ...
    pub(crate) start_eid_diff_cell: AllocatedU32Cell<F>,
    pub(crate) end_eid_diff_cell: AllocatedU32Cell<F>,
...
const U32_CELLS: usize = if cfg!(feature = "continuation") { 10 } else { 0 };
...
impl<F: FieldExt> EventTableCellAllocator<F> {
    ...
    pub(crate) fn alloc_memory_table_lookup_read_cell(
        ...
        let cells: Vec<_> = self
            ...
            .into_iter()
            .map(|x| AllocatedUnlimitedCell(x))
            .collect();
        ...
        let cell = AllocatedMemoryTableLookupReadCell {
            ...
            encode_cell: cells[2],
            value_cell: cells[3],
            start_eid_diff_cell: self.alloc_u32_cell(),
            end_eid_diff_cell: self.alloc_u32_cell(),
        };
