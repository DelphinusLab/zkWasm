                    ...
                    .map(
                        |(
                            ...
                            self.config
                                .common_config
                                .input_index_cell
                                .assign(ctx, F::from(host_public_inputs as u64))?;
                            self.config
                                .common_config
                                .context_input_index_cell
                                .assign(ctx, F::from(context_in_index as u64))?;
                            self.config
                                .common_config
                                .context_output_index_cell
                                .assign(ctx, F::from(context_out_index as u64))?;
                            self.config
                                .common_config
                                .external_host_call_index_cell
                                .assign(ctx, F::from(external_host_call_call_index as u64))?;
                            self.config
                                .common_config
                                .sp_cell
                                .assign(ctx, F::from(entry.eentry.sp as u64))?;
                            self.config
                                .common_config
                                .mpages_cell
                                .assign(ctx, F::from(entry.eentry.allocated_memory_pages as u64))?;
                            ...
                            self.config
                                .common_config
                                .frame_id_cell
                                .assign(ctx, F::from(entry.eentry.last_jump_eid as u64))?;
                            self.config
                                .common_config
                                .eid_cell
                                .assign(ctx, F::from(entry.eentry.eid as u64))?;
                            self.config
                                .common_config
                                .fid_cell
                                .assign(ctx, F::from(entry.eentry.fid as u64))?;
                            self.config
                                .common_config
                                .iid_cell
                                .assign(ctx, F::from(entry.eentry.iid as u64))?;
                            self.config
                                .common_config
                                .itable_lookup_cell
                                .assign(ctx, bn_to_field(&instruction.encode))?;

                            let op_config =
                                op_configs.get(&((&instruction.opcode).into())).unwrap();
