...
impl Iterator for Slices {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let etable_entries = self.pop_etable_entries();
        ...
        let (updated_init_memory_table, updated_post_initialization_state) = {
            ...
            let next_entry = if self.remaining_etable_entries.is_empty() {
                None
            } else {
                self.remaining_etable_entries.first()
            };

            let updated_post_initialization_state = self
                .last_compilation_table
                .update_initialization_state(&etable_entries, next_entry);
