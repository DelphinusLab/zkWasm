...
impl<F: FieldExt> Sha256HelperTableConfig<F> {
    pub(super) fn arg_to_shift_u32_expr(
        ...
        let shift_acc: i32 = 0;
        let acc = nextn!(meta, self.args[index].internal, start);
    ...
    pub(super) fn arg_to_shift_u32_expr_with_lowest_u4(
        ...
        (
            ...
            nextn!(meta, self.args[index].internal, start),
        )
    }
    ...
    pub(self) fn u4_array_to_u32_expr(
        ...
        for i in start..end {
            shift_acc += 4;
            acc = acc
                + nextn!(meta, self.args[index].internal, i) * constant_from!(1u64 << shift_acc);
        }
        (acc, shift_acc)
    }
}
