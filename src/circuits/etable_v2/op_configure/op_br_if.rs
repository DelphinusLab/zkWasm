...
impl<F: FieldExt> EventTableOpcodeConfig<F> for BrIfConfig<F> {
    ...
    fn sp_diff(&self, meta: &mut VirtualCells<'_, F>) -> Option<Expression<F>> {
        Some(
            constant_from!(1)
                + (constant_from!(1) - self.cond_is_zero_cell.expr(meta))
                    * self.drop_cell.expr(meta),
        )
    }

    fn mops(&self, meta: &mut VirtualCells<'_, F>) -> Option<Expression<F>> {
        Some((constant_from!(1) - self.cond_is_zero_cell.expr(meta)) * self.keep_cell.expr(meta))
    }
    ...
    fn next_iid(
        ...
        Some(
            (constant_from!(1) - self.cond_is_zero_cell.expr(meta)) * self.dst_pc_cell.expr(meta)
